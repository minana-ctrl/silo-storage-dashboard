---
name: Implement Conversations Page with Voiceflow Transcripts API
overview: ""
todos:
  - id: ef15bc03-73cc-4e37-b38c-4c8fe7b2c118
    content: Define TypeScript types for transcript summaries, dialog turns, and conversation filters in `types/conversations.ts`.
    status: pending
  - id: 1e931790-4f5e-4c25-922a-392325f24bec
    content: Implement Voiceflow Transcripts API client helpers in `lib/voiceflowTranscripts.ts` (or extend `lib/voiceflow.ts`) for listing transcripts and fetching a single transcript dialog.
    status: pending
  - id: 4aa84c57-7b7f-414b-a8e7-97b96f24a6a4
    content: Create Next.js API routes under `app/api/conversations` to proxy the Voiceflow Transcripts API, handle filters and pagination, and normalize responses.
    status: pending
  - id: a7efc14d-d1a6-4ae0-942e-52c6b63e3758
    content: Build ConversationList, ChatInterface, MessageBubble, ConversationInspector, and PlatformIcon components to render the 3-column Conversations layout.
    status: pending
  - id: d8208ecf-df7d-4da5-9f02-e846b0466b56
    content: Update `app/conversations/page.tsx` to compose the new components, manage selection and filters, and apply dashboard layout styling.
    status: pending
---
...
- Renders search input ("Search user…"), time filter (Last 7/14/30/90 days), filter icon, export icon.
